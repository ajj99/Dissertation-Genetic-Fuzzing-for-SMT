...
def count_root_nodes_by_solver(formulas, solvers):
    """Count root node types per solver."""
    root_nodes = ["and_", "eq", "le", "lt", "ge", "gt"]
    counts = defaultdict(lambda: defaultdict(int))

    for formula, solver in zip(formulas, solvers):
        root = re.match(r'[A-Za-z_][A-Za-z0-9_]*', formula).group()
        if root in root_nodes:
            counts[solver][root] += 1

    solvers_found = sorted(counts.keys())
    header = f"{'root':<8}" + "".join(f"{s:>10}" for s in solvers_found) + f"{'total':>10}"
    print(header)
    print("-" * len(header))
    ...
    for root in root_nodes:
        row = f"{root:<8}"
        row_total = 0
        for solver in solvers_found:
            n = counts[solver][root]
            row += f"{n:>10}"
            totals[solver] += n
            row_total += n
        row += f"{row_total:>10}"
    ...
    print("-" * len(header))
    print(f"{'total':<8}" + "".join(f"{totals[s]:>10}" for s in solvers_found) + f"{sum(totals.values()):>10}")


# Tokenisation

def tokenize_formula(deap_str):
    """Split a DEAP formula into its smallest meaningful tokens,
    excluding ARG variables (ARG0, ARG1, ..., ARGn).
    """
    tokens = re.findall(r'[(),]|[A-Za-z_][A-Za-z0-9_]*', deap_str)
    return [t for t in tokens if not re.match(r'^ARG\d+$', t)]


def build_vocabulary(all_formulas):
    """Build a sorted vocabulary of all unique tokens across all formulas."""
    vocab = set()
    for formula in all_formulas:
        vocab.update(tokenize_formula(formula))
    return sorted(vocab)


# Embedding

def embed_formula_bow(deap_str, vocab):
    """Embed a single formula as a bag-of-words count vector over the vocab."""
    counts = Counter(tokenize_formula(deap_str))
    return [counts.get(token, 0) for token in vocab]


def build_embeddings(formulas):
    """Embed all formulas. Returns (embeddings, vocab)."""
    vocab = build_vocabulary(formulas)
    print(f"Vocabulary ({len(vocab)} tokens): {vocab}")

    embeddings = []
    for i, formula in enumerate(formulas):
        try:
            embeddings.append(embed_formula_bow(formula, vocab))
...
def encode_labels(solvers):
    le = LabelEncoder()
    return le.fit_transform(solvers), le
...
def plot_pca_umap_sidebyside(X, solvers, formulas):
    """Plot PCA and UMAP side by side, coloured by solver, labelled by root node."""

    root_labels = [re.match(r'[A-Za-z_][A-Za-z0-9_]*', f).group() for f in formulas]
    ...
    axes[1].legend(handles=legend_handles, loc="lower left", fontsize=12, frameon=True)
...
if __name__ == "__main__":
    ...
    embeddings, vocab = build_embeddings(deap_formulas)

    X = np.array([e for e in embeddings if e is not None])
    X = StandardScaler().fit_transform(X)

    plot_pca_umap_sidebyside(X, solvers, deap_formulas)